...
import CommentInput from '../Navcomp/Comments/CommentInput'
import ShortsComment from "../Navcomp/Comments/ShortsComment";
const Shorts = () => {
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            const intersectionIndex = Array.from(refItem.current).indexOf(
              entry.target
            );
  ...
  useEffect(() => {
    if (currentShortIndex !== null) {
      setMuted((prevMuted) =>
        prevMuted.map((value, index) =>
          index === currentShortIndex ? false : true
        )
      );
    }
  }, [currentShortIndex]);
  ...
  return (
    <div>
      {videosData
        .filter((items, index) => index > shortIndexData && items.duration < 60)
        .map((videoItem, videoindex) => {
          return (
            <div
              className="flex ref-point"
              key={videoindex}
              ref={(el) => (refItem.current[videoindex] = el)}
            >
              <div className="h-[580px] bg-neutral-800 z-[50] w-[330px] rounded-xl mb-2 mt-1 overflow-hidden relative">
                {isPlay[videoindex] ? (
                  <motion.div
                  ...
                  <div className="flex items-center">
                    <span>
                      <img
                        ...
                        className="h-[40px] w-[40px] bg-slate-950 rounded-full mr-2"
                        alt="icon"
                      />
                    </span>
                    <span>@ApnaCollgeOfficial</span>
                    <button className="bg-yellow-50 text-neutral-800 rounded-[30px] py-2 px-3 ml-2">
                      Subscribe
                    </button>
                  </div>
                  <div>
                    <span className="font-[600] mt-1">
                      3 Best Code Editors for Programmers | Software Engineers
                    </span>
                  </div>
                </div>
              </div>
              <div className="  flex flex-col justify-end items-center ml-4">
                <div className="">
                  <div className="flex items-center justify-center text-4xl font-thin cursor-pointer w-12 h-12 bg-neutral-200 rounded-full hover:bg-neutral-300 p-3">
                    <BiSolidLike />
                  </div>
                ...
                <div onClick={()=>setComment(!iscomment)} className="">
                  <div className="flex items-center justify-center text-4xl font-thin cursor-pointer w-12 h-12 bg-neutral-200 rounded-full hover:bg-neutral-300 p-3">
                    <MdInsertComment />
                  </div>
                  <div className="flex items-center justify-center w-10 h-10">
                ...
                <div className="">
                  <img
                    src="https://yt3.ggpht.com/nhDLqeIgXMJBDIrX2bXavvHoWX0tsslDEh7k2xZ1P0W8b_CMRVigp2kxJubYEVwBcBlogZDe=s88-c-k-c0x00ffffff-no-rj"
                    className="h-[40px] w-[40px] bg-slate-950 rounded-lg mb-3 mt-2"
                    alt="icon"
                  />
        ...
        {
          iscomment &&
           <ShortsComment iscomment={iscomment} setComment={setComment}/>
        }
    </div>
  );
};
